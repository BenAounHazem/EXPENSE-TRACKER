...
function App() {
  ...
  const groupExpensesByLabel = (expenses) => {
    ...
    return Object.entries(groupedExpenses).map(([label, value]) => ({
      label,
      value,
    }));
  };
      ...
      <div className="flex flex-col justify-center items-center mt-[3%] w-[80%] mr-[5%] ml-[5%]">
        ...
        <div className="flex items-center justify-between mt-5 w-[100%]">
          <div className="relative flex justify-between w-[300px]">
            <button
              className="bg-teal-300 p-[10px] border-none outline-none cursor-pointer text-[#fff] text-medium"
              onClick={handleAddExpense}
            >
              Add Expense
            </button>
            <button
              className="bg-rose-300 cursor-pointer p-[10px] text-[#fff]"
            ...
            {addExpense && (
              <div className="absolute z-[999] flex flex-col p-[10px] top-[20px] left-0 h-[500px] w-[500px] bg-white shadow-xl">
                ...
                <button
                  className="bg-teal-300 text-white p-[10px] border-none cursor-pointer my-10"
            ...
            {showChats && (
              <div className="absolute z-[999] flex flex-col p-[10px] top-[20px] left-[100px] h-[500px] w-[500px] bg-white shadow-xl">
                ...
                <PieChart
                  series={[
                    {
                      data: groupExpensesByLabel(expenses), // Use grouped expenses
                      innerRadius: 30,
                      outerRadius: 100,
                      paddingAngle: 5,
                      cornerRadius: 5,
        ...
        <div className="flex flex-col">
          {filteredExpenses.map((expense, index) => (
            <>
              <div
                className="relative flex justify-between items-center w-[80vw] h-[100px] bg-[#FEF6C9] my-[20px] py-[10px]"
                key={index}
              >
                <h2 className="m-[20px] text-[#555] text-[18px] font-medium flex-1 text-left">
                  {expense.label}
                </h2>
                <h2 className="m-[20px] text-[18px] flex-1 text-center">{expense.date}</h2>
                <h2 className="m-[20px] text-[18px] font-medium flex-1 text-right">
                  ${expense.value}
                </h2>
